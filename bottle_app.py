...
def get_filter_cond():
	...
	for i in di.keys():
		if di[i]=='+':
			...
			<input size="45" type="search"  pattern=".{1,}" name="name" required ><br>
...
def filt_name(search,cs,data):
	if cs=='enabled':
		new_data=[row for row in data if search in row[0]]
	else:
		search=search.replace('İ','i').lower()
		new_data=[row for row in data if search in row[0].replace('İ','i').lower()]
	return new_data
...
def filter_result():
	...
	if len(data)==0:
		return a2.template %{'title':"Filter Result",'navbar':navbar,'form':"",'table':"<p>No records were found that match the specified search crieteria :( <a href=\"/search\"> See full table...</a></p>"}
	data=meta+data
